...
try {
    ...
    $CLUSTER_POWER_STATE = az aks show --name $AKS_CLUSTER_NAME --resource-group $RESOURCE_GROUP --query "powerState.code" --output tsv
    
    Write-Host "📊 Cluster Details:" -ForegroundColor Blue
    Write-Host "  Provisioning State: $CLUSTER_STATE"
    Write-Host "  Power State: $CLUSTER_POWER_STATE"
    Write-Host ""
    ...
    elseif ($CLUSTER_STATE -eq "Succeeded") {
        ...
        Write-Host ""
        Write-Host "   If you're seeing import errors, consider:"
        Write-Host "   1. Configuration mismatch between Terraform and existing cluster"
        Write-Host "   2. Updating Terraform configuration to match existing cluster"
        Write-Host "   3. Using force import instead of deletion"
        Write-Host ""
...
Write-Host "  ✓ Delete the node resource group: rg-aks-nodes-devops-eastus"
...
# Delete the cluster
Write-Host "   Running: az aks delete --name $AKS_CLUSTER_NAME --resource-group $RESOURCE_GROUP --yes"
try {
    ...
    if ($LASTEXITCODE -eq 0) {
        ...
        Write-Host "✓ New AKS cluster will be created with correct configuration"
        Write-Host "✓ Managed identity and role assignments already working"
        Write-Host "✓ All network and gateway resources already in place"
        Write-Host "✓ Complete deployment should succeed"
    }
    else {
        throw "Cluster deletion failed with exit code: $LASTEXITCODE"
    }
}
catch {
    Write-Host ""
    Write-Host "❌ Failed to delete AKS cluster: $($_.Exception.Message)" -ForegroundColor Red
    Write-Host ""
    Write-Host "Possible causes:" -ForegroundColor Yellow
    Write-Host "1. Permissions issue - ensure you have Contributor access"
    Write-Host "2. Cluster is locked or has delete protection"
    Write-Host "3. Azure service issue - try again in a few minutes"
    Write-Host ""
    Write-Host "Manual alternatives:" -ForegroundColor Yellow
    Write-Host "1. Try deletion via Azure Portal"
    Write-Host "2. Use Azure PowerShell: Remove-AzAksCluster"
    Write-Host "3. Contact Azure support if cluster is stuck"
    Write-Host ""
    exit 1
}
